# Ji Wook Choi - Sogang University - 2023
...
param21     = 20     # Accumulatoir threshold for circles, the lower the more false circles are recognised
minDiameter1  = 100    # Minimum radius of found circles in um
...
Npos = 0.0 # Counter for positive droplets, float so probability can be calculated
Nneg = 0.0 # Counter for negative droplets, float so probability can be calculated
...
def Histogram(Data):
    ...
    layout = go.Layout(
        title='Histogram of flourescence intensity of droplets',
...
c = conn.cursor()
c.execute('DROP TABLE IF EXISTS Droplets')
c.execute('DROP TABLE IF EXISTS Concentration')
c.execute('CREATE TABLE IF NOT EXISTS Droplets (ImmageNumber, Xcoordinate, Ycoordinate, Radius, Intensity)')
c.execute('CREATE TABLE IF NOT EXISTS Concentration (Threshold, PosDrop, NegDrop, Propability, LowBound, UppBound, Conc)')
conn.commit()
...
for i in range(0,Nimg):
    ...
    for j in circles[0,:]:
        c.execute('''INSERT INTO Droplets VALUES (?, ?, ?, ?, ?)''', (str(i+1) ,str(j[0]) ,str(j[1]) ,str(j[2]) ,str(intensity[k])))
        k = k+1
...
data = c.execute('''Select * FROM Droplets''')
...
while True:
    ...
    for x in plotdata:
        if (Threshold1 < x) & (x <= Threshold2):        # Droplets contianing now particle/cell have a higher intensity
            Nneg = Nneg + 1        
    ...
    Npart = Npos + Nneg # Total number of detected droplets
    ...
    c.execute(''' INSERT INTO Concentration VALUES (?, ?, ?, ?, ?, ?, ?)''', (Threshold2 , Npos, Nneg, pHat, int(C_low), int(C_upp), int(C_est)))
